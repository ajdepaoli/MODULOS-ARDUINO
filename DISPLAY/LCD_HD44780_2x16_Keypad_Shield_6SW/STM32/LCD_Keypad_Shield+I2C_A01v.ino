...
const int ledControlPB12 = PB12;              // LED en pin B12 (valor = -32,768 a 32,767)
//
// "IB03"
// Ж Ж Ж ENTRADAS ANALOGICAS Low/High o 0V/3.3V Ж Ж Ж
const int teclaA0 = PA0;		// Ingreso del Teclado del "Shield LCD Keypad"  (valor = -32,768 a 32,767)
// 
// "IC00"
...
// Variables del LED de Control en PB12
...
// Д Д Д Configuración de los Pines como SALIDAS Д Д Д
  ...
  pinMode(ledControlPB12, OUTPUT);				// LED de diagnostico
  digitalWrite(ledControlPB12, LOW);			// LED apagado
  pinMode(teclaA0, INPUT);				// Entrada del TECLADO.
...
// SVL01 "Parpadeo del LED"
  if (millis() - ultimoTiempoPB12 >= tiempoParpadeoB12) {
    ...
    digitalWrite(ledControlPB12, ledStatePB12 ? HIGH : LOW);
  } // "FIN" de SVL01
//
// SVL02 "Lectura del teclado y muestra el valor ADC"
  if (!mostrandoPA0) {									// Si es True, Salto a 'SLV02-A' ---- ('!' invertir)
    valorPA0 = analogRead(teclaA0);					// Valor de ADC según 'PA0'; y Salto a 'SVL02-C'
